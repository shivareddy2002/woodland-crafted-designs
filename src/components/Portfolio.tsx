...
import { MapPin, Quote, X, Award, Clock, Users, ChevronLeft, ChevronRight } from "lucide-react";
    ...
    <>
      <section id="portfolio" className="py-20 bg-background" ref={sectionRef as React.RefObject<HTMLElement>}>
        <div className="container mx-auto px-4 lg:px-6">
          ...
          <div className={cn(
            "relative transition-all duration-700 ease-out mb-16",
            isVisible ? "opacity-100 translate-y-0" : "opacity-0 translate-y-8"
            ...
            <div className="overflow-hidden mx-0 md:mx-8" ref={emblaRef}>
              <div className="flex -ml-4 md:-ml-6">
                {categories.map((category, index) => (
                  <div
                    key={category}
                    className="flex-[0_0_50%] md:flex-[0_0_33.333%] lg:flex-[0_0_16.666%] pl-4 md:pl-6 min-w-0"
                  >
                    <Card
                      onClick={() => handleCategoryClick(category)}
                      className="group cursor-pointer overflow-hidden hover-lift hover-glow border-2 border-border hover:border-primary bg-card transition-all duration-500 ease-out h-full"
                    >
                      <CardContent className="p-6 text-center">
                        <div className="mb-3">
                          <Award className="w-12 h-12 mx-auto text-primary group-hover:scale-110 transition-transform duration-300" />
                        </div>
